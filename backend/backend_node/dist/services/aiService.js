...
exports.askAgent = exports.executeMockAgent = exports.executeCheckoutCart = exports.executeAddToCart = exports.executeViewCart = exports.executeSearchCatalog = exports.getRecommendations = exports.generateEmbedding = void 0;
const axios_1 = __importDefault(require("axios"));
const db_1 = __importDefault(require("../config/db"));
const DEFAULT_HF_API_URL = 'https://router.huggingface.co/hf-inference/models/sentence-transformers/all-MiniLM-L6-v2/pipeline/feature-extraction';
const DEFAULT_GROQ_URL = 'https://api.groq.com/openai/v1/chat/completions';
const EMBED_MAX_RETRIES = 3;
const EMBED_RETRY_DELAY_MS = 2000;
const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
const vectorNorm = (vector) => Math.sqrt(vector.reduce((sum, value) => sum + value * value, 0));
/**
...
const generateEmbedding = async (text) => {
    ...
    for (let attempt = 1; attempt <= EMBED_MAX_RETRIES; attempt++) {
        ...
        try {
            const response = await axios_1.default.post(HF_API_URL, { inputs: text }, {
                headers: {
            ...
            console.error(`[HF EMBEDDING] FAILED after ${Date.now() - startedAt}ms (HTTP ${status || 'network error'}):`, error.response?.data || error.message);
 ...
 */
const getRecommendations = async (_userId, pastPurchases) => {
    try {
        ...
        const vector = await (0, exports.generateEmbedding)(pastPurchases.join(' '));
...
const executeSearchCatalog = async (query) => {
    try {
...
exports.executeSearchCatalog = executeSearchCatalog;
const executeViewCart = async (userId) => {
    try {
        const pendingOrder = await db_1.default.order.findFirst({
...
exports.executeViewCart = executeViewCart;
const executeAddToCart = async (userId, productId, quantity = 1) => {
    try {
...
exports.executeAddToCart = executeAddToCart;
const executeCheckoutCart = async (userId) => {
    try {
        const pendingOrder = await db_1.default.order.findFirst({
            where: { userId, status: 'PENDING' },
            include: { items: { include: { product: true } } }
...
exports.executeCheckoutCart = executeCheckoutCart;
// --- MOCK NLP SERVICE FALLBACK (For Keyless Execution) ---
const executeMockAgent = async (userId, userMessage) => {
    const query = userMessage.toLowerCase();
    // A. Checkout Intent
    if (query.includes('checkout') || query.includes('buy now') || query.includes('complete order')) {
        const res = await (0, exports.executeCheckoutCart)(userId);
        if (res.error) {
    ...
    if (addMatch) {
        ...
        // Find item
        const products = await (0, exports.executeSearchCatalog)(searchName);
        if (products.length > 0) {
            const match = products[0];
            const res = await (0, exports.executeAddToCart)(userId, match.id, 1);
            if (res.error) {
    ...
    if (query.includes('cart') || query.includes('bag') || query.includes('what did i add')) {
        const cart = await (0, exports.executeViewCart)(userId);
        ...
        const itemList = cart.items.map((i) => `- **${i.name}** (x${i.quantity}) - $${i.subtotal}`).join('\n');
        return {
    ...
    const products = await (0, exports.executeSearchCatalog)(userMessage);
    if (products.length === 0) {
        return {
...
exports.executeMockAgent = executeMockAgent;
// --- ACTIVE SERVICE ENTRYPOINT ---
const askAgent = async (userId, message, chatHistory) => {
    ...
    if (!GROQ_KEY) {
        ...
        return await (0, exports.executeMockAgent)(userId, message);
    }
    try {
...
- Keep responses concise and engaging.`;
        ...
        if (choice.tool_calls && choice.tool_calls.length > 0) {
            ...
            if (functionName === 'searchCatalog') {
                toolResult = await (0, exports.executeSearchCatalog)(args.query);
            ...
            if (functionName === 'viewCart') {
                toolResult = await (0, exports.executeViewCart)(userId);
                const listStr = toolResult.items.length === 0
                    ? "empty"
                    : toolResult.items.map((i) => `${i.name} (x${i.quantity})`).join(', ');
                return {
            ...
            if (functionName === 'addToCart') {
                toolResult = await (0, exports.executeAddToCart)(userId, args.productId, args.quantity || 1);
                if (toolResult.error) {
            ...
            if (functionName === 'checkoutCart') {
                toolResult = await (0, exports.executeCheckoutCart)(userId);
                if (toolResult.error) {
